...
<body>
	<div class="container content">
		<div class="row">
			<form id="upload" method="post" action="/mp4Controller/uploadImg" enctype="multipart/form-data">
		        <div>
		          <div>
		          	 <select name="fileContentType" id="fileContentType">
						 <c:forEach var="movieType" items="${movieList}"  varStatus="status">
							 <%--<c:if test="${movieType.type_id == 9}">--%>
								 <option value="${movieType.type_id}">${movieType.name}</option>
							 <%--</c:if>--%>
						 </c:forEach>
		          	 </select><br>
					  <input type="text" name="filename1" id="filename1"><br>
